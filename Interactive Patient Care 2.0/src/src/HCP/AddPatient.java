...
public class AddPatient extends JFrame {
	...
	public AddPatient() {
		...
		btnAddPatient.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				try{
					...
					if(count == 1){
						pst = connection.prepareStatement("CREATE TABLE IF NOT EXISTS viewPatientList(hcpID int NOT NULL, patientID int NOT NULL, FirstName TEXT NOT NULL, Surname TEXT NOT NULL, Age INT NOT NULL, Gender TEXT NOT NULL)");
						pst.executeUpdate();
						
						String s = Login.getHcpID();
						String s2 = textFieldID.getText();
						
						String query2="Insert into viewPatientList (hcpID,patientID,FirstName,Surname,Age,Gender) values (?,?,?,?,?,?)";
		...
		btnAddPatient.setBounds(246, 99, 137, 35);
